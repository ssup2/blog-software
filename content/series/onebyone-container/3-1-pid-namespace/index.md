...
[Shell 1]은 `nginx` Container 내부에서 본 Process를 나타내고 있고 [Shell 2]는 `httpd` Container 내부에서 본 Process를 나타내고 있다. 마지막으로 [Shell 3]은 `nginx` Container와 `httpd` Container를 구동한 Host에서 본 Process를 나타내고 있다. `nginx` Container와 `httpd` Container는 서로의 Process를 확인할 수 없지만, Host는 두 Container의 Proces를 모두 확인할 수 있다. 이러한 현상은 PID Namespace의 특징 때문에 발생한다.

...
Kernel Configuration 수정을 통해서 `virtio-scsi`를 활성화 한다.
...
VM이 이용할 기본 tty를 설정한다. `mnt`에 Mount된 `etc/init/ttyS0.conf` 파일을 [File 1]과 같이 수정한다.
...
VM의 Network를 설정한다. `mnt`에 Mount된 `etc/network/interfaces` 파일을 [File 2]와 같이 수정한다.
...
`rootfs.img`에 Kernel module을 설치한다.
...
`vhost-scsi`를 위한 LIO Package를 설치한다.
...
`vhost-scsi`를 위한 LIO를 설정한다. `/var/target/fabric/vhost.spec` 파일을 [File 4]와 같이 설정한다.
...
VM을 위해서 생성한 `rootfs.img`, `bzImage`를 복사한다.
...
User Networking (SLIRP), `virtio-net`, `virtio-blk`을 이용하여 VM을 생성한다.
...
TAP, `virtio-net`, `virtio-blk`을 이용하여 VM을 생성한다.
...
User Networking (SLIRP), `virtio-net`, `virtio-scsi`을 이용하여 VM을 생성한다.
...
TAP, `virtio-net`, `virtio-scsi`을 이용하여 VM을 생성한다.
...
TAP, `virtio-net`+`vhost`, `virtio-scsi`+`vhost-scsi`을 이용하여 VM을 생성한다.

...
`kubectl`을 이용하여 etcd Container에 진입한 이후, `etcdctl`을 이용하여 Key 조회 명령어를 수행한다. `ETCDCTL_API` 환경변수를 이용하여 API Version을 반드시 명시해야하고, 인증서 관련 파일도 Option을 통해서 반드시 지정해주어야 한다. `/` (root) 경로를 대상으로 조회 하였기 때문에 etcd가 갖고 있는 모든 Key를 조회한다.

...
Pod가 종료되도 Pod 안에서 동작하는 App Container가 안정적으로 Request을 처리하기 위해서는 Pod의 종료 과정을 완전히 이해하고 적절하게 Pod를 설정하는 과정이 필요하다. [Figure 1]은 상세한 Pod의 종료 과정을 나타내고 있다.
...
4. PreStop Hook의 동작이 완료되면 kubelet은 SIGTERM Signal을 전송한다. SIGTERM을 받은 App Container는 현재 처리중인 Request를 모두 처리하고 종료를 시도한다.
5. 만약 SIGTERM을 받은 App Container가 종료되지 않으면 SIGKILL Signal을 받고 강제로 종료된다. kubelet은 K8s API 서버가 받은 Pod 종료 요청 시간부터 App Container에 설정된 `terminationGracePeriodSeconds` 시간만큼 대기후에 SIGKILL Signal을 전송하며, `terminationGracePeriodSeconds`의 기본값은 30초이다.

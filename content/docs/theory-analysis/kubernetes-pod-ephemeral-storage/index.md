...
[File 3]과 같이 만약 **Memory** Medium의 **empyDir** Volume에 용량을 제약을 설정하지 않을수 있으며, 이 경우에는 최대 Container의 Memory Limit의 크기만큼 사용할 수 있다. [File 3]의 경우에는 `my-shell` Container의 Limit Memory가 `2Gi`이므로 최대 `2Gi`의 용량을 사용할 수 있다. 만약 Container에 Memory Limit을 설정하지 않으면 최대 Node의 Memory 크기만큼 사용할 수 있다.

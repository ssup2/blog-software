...
* `x-envoy-internal` : Indicates whether the request originated from the internal network, and is set to `true` for Internal requests. The reason Envoy distinguishes Internal requests is to **apply different trust levels depending on the origin of the request**. If Headers that let the Downstream Client control Envoy's behavior, such as the Timeout and Retry control Headers, were also allowed on External requests, a malicious external Client could arbitrarily manipulate the retry count or Timeout, so Envoy trusts these Headers only on Internal requests. Instead of directly authenticating the requester, Envoy infers trust from the network position the request came from, and determines whether a request is Internal based on the `x-forwarded-for` (XFF) Header and the `use_remote_address` setting as follows.
  * The XFF Header is a Header to which each Proxy appends, on the right, the IP address of the Client that connected to it as the request passes through Proxies, serving as a record of the network path the request has traveled.
  * The `use_remote_address` setting decides which of the two — the IP address of the directly connected Downstream or the XFF Header — to trust as the basis for the determination. It is set to `true` when Envoy runs as an Edge Proxy facing the outside, where the directly connected Downstream is the actual Client, and to `false` when Envoy runs as an internal Proxy with other Proxies in front, where the path must be understood through the XFF Header.

...
* `x-envoy-internal` : 요청이 내부 네트워크에서 유입된 요청인지를 나타내며, Internal 요청인 경우 `true` 값이 설정된다. Envoy가 Internal 요청을 구분하는 이유는 요청의 출처에 따라서 **신뢰 수준을 다르게 적용**하기 위함이다. Timeout, Retry 제어 Header처럼 Downstream Client가 Envoy의 동작을 제어하는 Header를 External 요청에도 허용하면 외부의 악의적인 Client가 재시도 횟수나 Timeout을 임의로 조작할 수 있기 때문에, Envoy는 이러한 Header를 Internal 요청에서만 신뢰한다. 이때 Envoy는 요청자를 직접 인증하는 대신 요청이 유입된 네트워크 위치로 신뢰 여부를 추정하며, 그 근거로 `x-forwarded-for` (XFF) Header와 `use_remote_address` 설정을 이용하여 Internal 요청 여부를 다음과 같이 판단한다.
  * XFF Header는 요청이 Proxy를 거칠 때마다 각 Proxy가 자신에게 연결한 Client의 IP 주소를 오른쪽에 추가하는 Header이며, 요청이 거쳐온 네트워크 경로의 기록에 해당한다.
  * `use_remote_address` 설정은 판단의 근거로 직접 연결된 Downstream의 IP 주소와 XFF Header 중 무엇을 신뢰할지 결정한다. Envoy가 외부와 맞닿은 Edge Proxy로 동작하여 직접 연결된 Downstream이 곧 실제 Client인 경우 `true`로 설정하고, 앞단에 다른 Proxy가 존재하여 XFF Header로 경로를 파악해야 하는 내부 Proxy로 동작하는 경우 `false`로 설정한다.

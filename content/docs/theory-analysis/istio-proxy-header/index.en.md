...
* Control Headers Not Working : Control Headers such as `x-envoy-upstream-rq-timeout-ms` and `x-envoy-retry-on` that the Client sets in requests do not work. Since the Sidecar Proxy's Envoy setting `use_remote_address` is `false` and the request sent by the Client does not contain an XFF Header, the Sidecar Proxy judges the request as an External request and removes control Headers with the `x-envoy-` Prefix. The Ingress Gateway also removes control Headers with the `x-envoy-` Prefix from External requests coming from outside the Mesh in the same way. Therefore, Timeout and Retry must be configured through the Virtual Service, not through control Headers.
...
As shown in [Shell 12], the request Headers that istio-ingressgateway received from the External Client are checked. As shown in [Text 8], only the `Host`, `User-Agent`, and `Accept` Headers sent by `curl` exist, and no Istio related Headers exist yet.

#### 1.4.2. Request Headers between Istio Proxies Case (istio-ingressgateway → mock-server istio-proxy)

```shell {caption="[Shell 13] Checking Request Headers between Istio Proxies"}
$ kubectl logs -n istio-system deploy/istio-ingressgateway | sed -n '/router decoding headers/,/thread=/p'
```

```text {caption="[Text 9] Request Headers Sent by istio-ingressgateway to mock-server istio-proxy"}
router decoding headers:
':authority', 'mock-server.example.com'
':path', '/status/200'
':method', 'GET'
...
As shown in [Shell 13], the request Headers that istio-ingressgateway sends to the `mock-server` istio-proxy are checked. As shown in [Text 9], the `x-request-id`, `x-envoy-decorator-operation`, and `x-envoy-peer-metadata` Headers are added in the same way as the Sidecar Proxy Case (1.3.2), with the following differences.
...
```shell {caption="[Shell 14] Checking Server Received Request Headers"}
$ kubectl logs mock-server -c istio-proxy | sed -n '/router decoding headers/,/thread=/p'
```

```text {caption="[Text 10] Request Headers Sent by mock-server istio-proxy to mock-server"}
router decoding headers:
':authority', 'mock-server.example.com'
':path', '/status/200'
':method', 'GET'
...
As shown in [Shell 14], the request Headers that the `mock-server` istio-proxy sends to the `mock-server` Container are checked. As shown in [Text 10], the Istio Proxy dedicated Headers are removed and the XFCC Header is added, while the `x-forwarded-for` and `x-envoy-external-address` Headers are maintained and delivered as is. The SPIFFE ID of istio-ingressgateway (`spiffe://cluster.local/ns/istio-system/sa/istio-ingressgateway-service-account`) is set in the `URI` Key of the XFCC Header. In other words, from the perspective of the `mock-server` Container, the Identity that can be verified through XFCC is istio-ingressgateway, not the original Client.

#### 1.4.4. Server Sent Response Header Case (mock-server → mock-server istio-proxy)

```shell {caption="[Shell 15] Checking Server Sent Response Headers"}
$ kubectl logs mock-server -c istio-proxy | sed -n '/upstream response headers/,/thread=/p'
```

```text {caption="[Text 11] Response Headers Received by mock-server istio-proxy"}
end_stream: false, upstream response headers:
':status', '200'
'content-type', 'application/json'
'content-length', '59'
...
As shown in [Shell 15], the response Headers that the `mock-server` istio-proxy received from the `mock-server` Container are checked. As in the Sidecar Proxy Case (1.3.4), the response sent by the `mock-server` Container contains only basic Headers.

#### 1.4.5. Response Headers between Istio Proxies Case (mock-server istio-proxy → istio-ingressgateway)

```shell {caption="[Shell 16] Checking Response Headers between Istio Proxies"}
$ kubectl logs mock-server -c istio-proxy | sed -n '/encoding headers via codec/,/thread=/p'
```

```text {caption="[Text 12] Response Headers Sent by mock-server istio-proxy to istio-ingressgateway"}
encoding headers via codec (end_stream=false):
':status', '200'
'content-type', 'application/json'
...
As shown in [Shell 16], the response Headers that the `mock-server` istio-proxy sends to istio-ingressgateway are checked. As in the Sidecar Proxy Case (1.3.5), the Metadata Headers of the `mock-server` Workload and the `x-envoy-upstream-service-time` and `server` Headers are added.

#### 1.4.6. Client Received Response Header Case (istio-ingressgateway → External Client)

```shell {caption="[Shell 17] Checking Client Received Response Headers"}
$ kubectl logs -n istio-system deploy/istio-ingressgateway | sed -n '/encoding headers via codec/,/thread=/p'
```

```text {caption="[Text 13] Response Headers Sent by istio-ingressgateway to External Client"}
encoding headers via codec (end_stream=false):
':status', '200'
'content-type', 'application/json'
...
As shown in [Shell 17], the response Headers that istio-ingressgateway delivers to the External Client are checked. As shown in [Text 13], the Istio Proxy dedicated Headers are removed, and the `x-envoy-upstream-service-time` Header is overwritten with the value measured by istio-ingressgateway (`2`). Unlike the Sidecar Proxy, the `server` Header is not changed to the `envoy` value, and the `istio-envoy` value is delivered as is.
...
[File 5] shows the Manifest that changes the `numTrustedProxies` setting of the Ingress Gateway to `1`. Istio changes Envoy's `xff_num_trusted_hops` setting through the `gatewayTopology.numTrustedProxies` setting. It means an environment where a trusted Proxy (AWS ALB, Nginx, etc.) exists in front of the Ingress Gateway.

```shell {caption="[Shell 18] Sending Request with xff_num_trusted_hops Configuration"}
...
```text {caption="[Text 14] Request Headers Sent to mock-server with xff_num_trusted_hops Configuration"}
...
As shown in [Shell 18], a request with two addresses (`1.2.3.4, 5.6.7.8`) set in the XFF Header is sent, and the request Headers sent to the `mock-server` Container are compared before and after the configuration. [Text 14] shows the results before and after the configuration.

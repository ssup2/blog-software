...
* **Sequence Number** : Unique sequential number of Records assigned by Producer. Generally, one Producer Request contains Record Batches by Partition/Topic, and each Record Batch contains **Base Sequence Number** that represents the start of Records and **Offset Sequence Number** that represents the difference from Base Sequence Number to the last Record. Sequence Number is a separate value from Offset used by Consumer. Sequence Number is a value assigned by Producer and used by Kafka Broker, while Offset is a value assigned by Kafka Broker and used by Consumer.

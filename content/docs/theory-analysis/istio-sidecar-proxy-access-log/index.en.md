...
In this state, the `shell` Pod's `istio-proxy` that received the TCP RST Flag terminates the Upstream Connection, but unlike the Downstream TCP RST Case, it closes a Socket with unread data remaining in the Kernel Receive Buffer, so according to TCP rules, a TCP RST Flag is sent to the `mock-server` Pod instead of a TCP FIN Flag. In other words, the Flag that `istio-proxy` sends when terminating the Upstream Connection is determined by whether unread data exists in the Kernel Receive Buffer at the time of termination.

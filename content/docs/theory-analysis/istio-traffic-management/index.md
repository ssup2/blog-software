...
`version-v1-v2` Virtual Service가 `version`을 Host로 이용할 수 있는 이유는 `version` Service가 선언되어 있기 때문이다. `version` Service를 통해서 실제로 Traffic이 Routing이 되는것은 아니지만 `version` Service가 존재하지 않으면 `version-v1-v2` Virtual Service가 `version`을 Host로 이용할 수 없기 때문에, 반드시 `version` Service도 선언되어 있어야 한다.
...
Virtual Service를 Gateway에 연결하기 위해서는 Virtual Service의 `hosts`에는 Gateway에서 설정한 Host를 추가해야 한다. 또한  Virtual Service의 `gateways`에는 Virtual Service가 연결된 Gateway의 이름을 명시해야 한다.

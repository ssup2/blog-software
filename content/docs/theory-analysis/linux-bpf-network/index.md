...
[Figure 1]는 Linux에서 제공하는 BPF Program Type 중에서 Network과 연관된 Type을 Kernel의 Network Stack과 함께 나타내고 있다. Network Stack별로 다양한 Type의 BPF Program이 존재하며 앞으로도 계속 추가될 예정이다. [Figure 1]은 **Kernel Version v5.13**을 기준으로 하고 있다.
...
* `XDP-ABORTED` : 해당 Packet을 버리고, `trace-xdp-exception`을 발생시킨다.

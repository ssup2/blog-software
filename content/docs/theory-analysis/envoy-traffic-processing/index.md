...
* `original_src` : 원래 클라이언트의 출발지 IP를 보존해, upstream으로 나갈 때 그 IP를 소스로 사용하도록 한다. 주로 original_dst와 짝지어 쓴다.
* `proxy_protocol` : PROXY protocol을 사용하는 경우, 맨 앞 PROXY 헤더를 읽어 원래 클라이언트 주소를 복원하고 그 헤더를 제거한다.
...
Listener Filter Chain은 Envoy Config에 따라서 자유롭게 구성할 수 있지만, 일반적으로 Proxy Header를 제거하기 위한 `proxy_protocol` 필터를 첫 번째로 사용하며, 

...
위와 같은 문제를 막기 위해선는 2개의 Client가 완전히 동일한 Timestamp를 갖고 있어야 한다. 하지만 다수의 Client가 완전히 동일한 Timestamp를 갖는것은 불가능하다. 따라서 Linux Manpage에서도 tcp-tw-recycle은 Client가 SNAT 되는 Network 환경에서 이용하지 않는것을 권장하고 있다.

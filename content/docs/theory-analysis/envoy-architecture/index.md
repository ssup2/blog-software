...
[Figure 2]는 Envoy TLS (Thread Local Storage)를 나타내고 있다. Main Thread와 Worker Thread는 자신만의 TLS를 가지고 있으며, Envoy는 TLS를 이용하여 성능 최적화를 위해서 Lock 활용을 최소화하며 동작한다. TLS는 CPP STL의 **vector Class**를 이용하여 Array 형태로 관리된다. [Figure 2]에서 알수 있는 것처럼 Index별로 용도가 정해져 있으며, Main/Worker Thread의 Index별 용도는 모두 동일하다. 이러한 이유는 Main Thread에서 Index별 용도를 할당하고 초기화한 다음에, Worker Thread도 동일하게 Index별 용도를 할당하고 초기화하기 때문이다.

TLS에 저장된 실제 데이터는 `shared_ptr` Pointer이며, Pointer를 따라 실제 객체(데이터)를 참조하게 된다. TLS 용도에 따라서 `shared_ptr` Pointer가 가리키는 객체는 Main/Worker Thread 사이에서 공유되거나, 공유되지 않고 각 Thread마다 독립적인 객체를 가지고 있을 수 있다. [Figure 2]에서는 Runtime, RouteConfig, DNS Cache의 경우에는 전자의 경우를 의미하며, ClusterManager, Stats의 경우에는 후자의 경우를 의미한다.

Thread 사이에 공유되는 객체를 갱신할 때는 일반적으로 변경 사항을 반영한 새 객체와 그에 대한 `shared_ptr` Pointer를 생성한 뒤, TLS에 저장된 기존 `shared_ptr` Pointer를 새 `shared_ptr` Pointer로 대체하여 Atomic 하게 갱신한다. 단 각 Thread 마다 TLS마다 존재하기 때문에 모든 TLS를 동시에 갱신하지는 않으며, 따라서 일시적으로 각 Thread는 서로 다른 객체를 참조할 수 있다. [Figure 2]에서 Runtime의 경우 모든 Thread들이 동일한 객체를 참고하고 있지만, RouteConfig, DNS Cache의 경우에는 데이터가 변경되면서 일시적으로 다른 객체를 참조하고 있는 모습을 나타내고 있다. 이러한 변경 방식은 Kernel의 **RCU (Read Copy Update)** 기법과 유사한 방식이다.

`shared_ptr` Pointer가 각 Thread마다 독립적인 객체를 가리키고 있는 경우에도, 독립적인 객체가 다시 Thread 사이의 공유 객체를 참조할 수 도 있다. [Figure 2]에서 ClusterManager의 경우 각 Thread마다 ClusterEntry가 존재하지만 다시 Thread 사이에 공유되는 ClusterInfo를 참조하고 있는 모습을 나타내고 있으며, Stats의 경우에도 유사하게 각 Thread마다 독립적인 ScopeCache가 존재하지만 다시 Thread 사이에 공유되는 Counter가 존재한다.
...
2. `cb` Callback 함수를 `cb_guard` Pointer에 저장한다. `cb_guard` Pointer는 `all_threads_complete_cb` Callback 함수를 Main Thread에 Posting 및 `cb` Callback 함수를 제거하는 Lambda를 Deleter로 등록한 CPP STL의 `shared_ptr` 역할을 수행한다.
...
5. 실행이 완료된 Lambda는 종료되며 이때 `cb_guard` Pointer의 Reference Count가 1씩 감소하게 되며, 마지막 Lambda가 종료되면 `cb_guard` Pointer의 Reference Count가 `0`이 된다.
...
Flush Thread/Buffer/Lock 세트는 각 Access Logger 마다 독립적으로 존재한다. [Figure 1]에서는 [File 1]과 같이 File과 stdout 각각의 경로에 출력하는 2개의 Access Logger가 존재하기 때문에 2개의 Flush Thread/Buffer/Lock 세트가 존재하는 것을 확인할 수 있다. 이 의미 Worker Thread에서도 Access Logger의 개수만큼 여러번 `Write` 함수를 호출하는 것을 의미하며, 따라서 너무 많은 Access Logger가 설정되어 있을 경우 성능 저하가 발생할 수 있다.

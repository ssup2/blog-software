...
User Level에서 Kernel Log를 얻기위해 이용되는 `syslog(2)` 함수(System Call)나 `/proc/kmsg` 파일은 `do_syslog()` 함수를 통해 Ring Buffer에 접근하여 Kernel Log를 얻는다. `dmesg`는 `syslog(2)` 함수를 통해 Kernel Log를 User가 볼 수 있도록 한다. 또한 `rsyslogd`나 `systemd-journald`는 `syslog(2)`나 `/proc/kmsg`를 통해서 Kernel Log를 `/var/log` 폴더에 파일로 기록하여 보관되도록 한다.

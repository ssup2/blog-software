...
`If-Modified-Since` Header는 Client가 특정 날짜, 시간 이후에 변경된 Resource만 받아오기 위해 사용되는 Header이다. [Text 11]은 `If-Modified-Since` Header의 Format을 나타낸다.
...
[Text 28]은 `X-Forwarded-Proto` Header의 몇가지 예시를 나타낸다.

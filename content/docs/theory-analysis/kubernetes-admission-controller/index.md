...
Custom Admission Controller는 Kubernetes API Server 외부에서 동작하는 Kubernetes User가 개발한 Admission Controller를 의미한다. Custom Admission Controller의 동작을 이해하기 위해서는 Compiled-in Admission Controller인 `MutatingAdmissionWebhook` Controller와 `ValidatingAdmissionWebhook` Controller의 역할을 이해하고 있어야한다. [Figure 1]은 `MutatingAdmissionWebhook` Controller와 `ValidatingAdmissionWebhook` Controller의 동작도 나타내고 있다.

`MutatingAdmissionWebhook` Controller는 전달받은 API 요청 정보를 등록된 Custom Mutating Admission Controller에 Webhook으로 전달하고 응답을 대기하는 역할을 수행한다. 다수의 Custom Mutating Admission Controller가 존재할 경우, 전달받은 API 요청 정보를 하나씩 순차적으로 Custom Mutating Admission Controller에게 전달하고 응답을 대기하는 동작을 반복한다. 
...
[Data 3]은 Mutating Custom Admission Controller에서 API 요청을 변경해야 하는경우 Mutating Custom Admission Controller가 Kubernetes API Server에게 전송하는 응답을 나타내고 있다. API 요청의 변경 내역은 JSON의 변경 내역을 저장하는 형태인 `JSONPatch`를 Base64 형태로 Encoding하여 `patch` 항목에 저장한다.

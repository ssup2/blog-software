...
AWS의 ARN (Amazon Resource Name)을 정리한다.
...
**AWS ARN**은 AWS에서 관리하는 Resource의 이름을 의미한다. [Text 1]은 AWS ARN의 Format들을 나타내고 있다. 총 3가지의 Format이 존재하며 `Resource-ID`만 존재하는 한가지의 Format과 `Resource-Type`, `Resource-ID`만 존재하는 2가지의 Format이 존재한다. 필요에 따라서 몇몇 ARN의 경우에는 `Region`, `Account-ID`가 생략될 수 있다. AWS ARN Format의 각 구성 요소들은 다음과 같다.

* `Partition` : AWS Region의 Group을 의미한다. `aws`, `aws-cn` (중국), `aws-us-gov` (미국 정부) 3개의 Partition이 존재하며, 대부분의 경우에는 `aws` Partition에 소속된다.

...
Consumer가 Message를 정상적으로 수신하였어도, Consumer에 의해서 Message는 `reject`또는 `nack` 될수 있다. `reject`는 하나의 Message에 대해서만 거절하는 응답이고, `nack`는 RabbitMQ에게 ACK를 전송하지 않은 모든 Message에 대해서 거절하는 응답이다. 거절된 Message는 `reject`/`nack` 응답과 함께 전송된 `requeue` 옵션에 따라서 처리가 달라진다.

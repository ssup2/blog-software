...
Map Interface는 **key-Value Group** 관리에 필요한 **기본적**인 Interface를 제공한다. Key는 중복될 수 없다. Group의 크기(`size()`), Key-Value 추가(`put()`), Value 얻기(`get()`), Key-Value 삭제(`remove()`), Set Interface 얻기(`entrySet()`, `keySet()`)등의 Method를 제공한다. Map Interface에서는 Iterator를 제공하지 않는다. `entrySet()`, `keySet()` Method를 통해 얻은 Set Interface의 Iterator를 이용한다.
